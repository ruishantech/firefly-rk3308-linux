...
static void *run_ffplay_thread(void *arg)
{
    ...
    char media_convertion[256] = {0};
    if(ipc_state.media_type == MEDIA_VIDEO){
        snprintf(media_convertion, sizeof(media_convertion)/sizeof(char), "-vcodec h264");
    }

    if (ipc_state.start_time > 0) {
        snprintf(cmd, sizeof(cmd), "/usr/bin/rkffplay -hide_banner -nostats %s -fs -ss %d \"%s\" &", media_convertion, ipc_state.start_time, file_path);
    } else {
        snprintf(cmd, sizeof(cmd), "/usr/bin/rkffplay -hide_banner -nostats %s -fs \"%s\" &", media_convertion, file_path);
    }
    printf("%s\n", cmd);
    ret = system(cmd);
    if (ret)
