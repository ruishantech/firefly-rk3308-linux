...
int SetLedState(int status)
{
	// /sys/class/leds/firefly:red:power/brightness
	// /sys/class/leds/firefly:red:power/trigger
	// /sys/class/leds/firefly:blue:user/trigger
	if (access("/sys/class/leds/firefly:red:power/brightness", F_OK) != 0) {
		return -1;
	}
	if (access("sys/class/leds/firefly:red:power/trigger", F_OK) != 0) {
		return -1;
	}
	if (access("/sys/class/leds/firefly:blue:user/trigger", F_OK) != 0) {
		return -1;
	}
	...
	switch(status) {
		case 0:
			...
			system("echo timer > /sys/class/leds/firefly:blue:user/trigger");
			break;

		case 1:
			// update ok
			system("echo 0 > /sys/class/leds/firefly:red:power/brightness");
			system("echo default-on > /sys/class/leds/firefly:blue:user/trigger");
			break;

		case -1:
			// update fail
			system("echo timer > /sys/class/leds/firefly:red:power/trigger");
			system("echo timer > /sys/class/leds/firefly:blue:user/trigger");
			break;
