...
void Launcher::pickWallpaper(QString path)
{
    ...
    if (QFile::exists("/usr/local/QLauncher/background.bmp"))
    {
        QFile::remove("/usr/local/QLauncher/background.bmp");
    }

    bool result=QFile::copy(QUrl(path).toLocalFile(), "/usr/local/QLauncher/background.bmp");
