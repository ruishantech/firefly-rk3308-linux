...
static void* run_ffplay_thread(void *arg) {
    ...
    if (ipc_state.start_time > 0) {
        snprintf(cmd, sizeof(cmd), "/usr/bin/rkffplay -vcodec h264 -hide_banner -nostats -an -fs -ss %d \"%s\" &", ipc_state.start_time, file_path);
    } else {
        snprintf(cmd, sizeof(cmd), "/usr/bin/rkffplay -vcodec h264 -hide_banner -nostats -an -fs \"%s\" &", file_path);
    }
    printf("%s\n", cmd);
    ret = system(cmd);
    if (ret) {
