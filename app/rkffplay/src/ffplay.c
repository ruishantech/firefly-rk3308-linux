...
static int ffplay_rockchip_rv1108_display(Frame *vp)
{
	...
	if (vp->frame->format != AV_PIX_FMT_DRM_PRIME) {
		...
		if (!g_rockchip_rga_fd) {
			printf("===== rk_rga_open =====\n");
			ret = rk_rga_open();
			if (ret < 0) {
				av_log(NULL, AV_LOG_ERROR, "Librkrga: open failed!\n");
				return -1;
			}
			av_log(NULL, AV_LOG_DEBUG, "Librkrga: open rkrga sucess!\n");
			g_rockchip_rga_fd = ret;
		}
		...
		ret = rk_rga_ionfd_to_ionfd_rotate(g_rockchip_rga_fd,
			src_fd, vp->width, vp->height, src_fmt, vir_w, vir_h,
			dst_fd, disp_width, disp_height, dst_fmt,
			rotate_angle);
		if (ret) {
			av_log(NULL, AV_LOG_ERROR, "Librkrga: rk_rga_ionfd_to_ionfd_rotate failed!"
				"\n\tapi input param:(rga_fd=%d)"
...
/*
    for (int i = 0; i < FF_ARRAY_ELEMS(supported_formats) - 1; i++) {
        if (pixFormat == supported_formats[i].pixfmt) {
            drm_format = supported_formats[i].drm_format;
            return;
        }
    }
*/
    //printf("pixformat 0x%x, dst_format 0x%x, drm_format 0x%x, %dx%d\n", pixFormat, dst_format, drm_format, frame->width, frame->height);

    VideoInfo vinfo;
    vinfo.format = drm_format;
    vinfo.width = frame->width;
    vinfo.height = frame->height;

    if (drm_inited == 0) {
        drm_inited = 1;
	memset(&kmssink, 0, sizeof(kmssink));
        kms_sink_start(&kmssink, &vinfo);
	kms_sink_set_render_rectangle(&kmssink, 0, 0, -1, -1);
        av_log(NULL, AV_LOG_FATAL, "fmt 0x%x, %d x %d, linesize %d %d %d\n", frame->format, vinfo.width, vinfo.height,
    ...
    if (++count % 150 == 0) {
        ...
        long elapsed = (stopTime.tv_sec-startTime.tv_sec) * 1000 + (stopTime.tv_usec - startTime.tv_usec) / 1000;
        av_log(NULL, AV_LOG_FATAL,"fps = %d\n", 150 * 1000 / elapsed);
        startTime = stopTime;
    }
    if (frame->format == AV_PIX_FMT_YUV420P ||
        frame->format == AV_PIX_FMT_YUVJ420P) {
        //av_log(NULL, AV_LOG_FATAL, "fmt 0x%x, linesize %d %d %d\n", frame->format, frame->linesize[0], frame->linesize[1], frame->linesize[2]);
    } else {
        //av_log(NULL, AV_LOG_FATAL, "fmt 0x%x\n", frame->format);
        return;
    }
    KMSMEM *buffer = kms_sink_get_mem(&kmssink, &vinfo);

    is->img_convert_ctx = sws_getCachedContext(is->img_convert_ctx,
            frame->width, frame->height, pixFormat, frame->width, frame->height,
            dst_format, NULL, NULL, NULL, NULL);
    if (is->img_convert_ctx != NULL) {
        uint8_t *pixels[4] = {0};
        int pitch[4] = {0};
        pixels[0] = buffer->bo->ptr;
        pixels[1] = buffer->bo->ptr + frame->width * frame->height;
        pitch[0] = frame->width;
        pitch[1] = frame->width;
	//printf("pixels[0] %p, linesize %d\n", pixels[0], frame->linesize[0]);
...
int main(int argc, char **argv)
{
    ...
    if (g_rockchip_rga_fd) {
        av_log(NULL, AV_LOG_DEBUG, "Librkrga: close rkrga sucess!\n");
        rk_rga_close(g_rockchip_rga_fd);
    }
#endif
    /* never returns */
